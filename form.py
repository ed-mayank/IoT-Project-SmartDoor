...
enter_cnt="http://127.0.0.1:8080/~/in-cse/in-name/Door_Accessed/Data"
...
def submit():
    ...
    for x in details:
        ...
        if Name==y[0] and Password==y[1]:
            print("Matched")
            create_data_cin(enter_cnt,Name)
...
passEntry=Entry(root, textvariable = passval, font = ('calibre',20,'normal'), show = '*')
...
refresh_btn=Button(root,text="Refresh",command=refresh).grid(row=8,column=1)
